...
# Описание программы
SamWaf — это лёгкий межсетевой экран для веб-приложений с открытым исходным кодом, предназначенный для небольших компаний, студий и персональных сайтов. Он поддерживает полностью приватное развёртывание, шифрует данные, хранящиеся локально, прост в запуске и работает на Linux, Windows 64-bit и ARM64; также доступны образы Docker. По умолчанию используется встроенная зашифрованная база данных SQLite без каких-либо внешних зависимостей, при необходимости можно переключиться на MySQL / PostgreSQL.
...
- SQLite (с шифрованием) по умолчанию, опционально MySQL / PostgreSQL, со встроенным инструментом миграции SQLite→MySQL / SQLite→PostgreSQL / MySQL→PostgreSQL
...
| `migratedb` | Офлайн-миграция базы данных SQLite → MySQL / SQLite → PostgreSQL / MySQL → PostgreSQL (`--dry-run` — только оценка, `--force` — перезапись) |
