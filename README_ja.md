...
# ソフトウェア紹介
SamWaf は、小規模企業・スタジオ・個人サイト向けの軽量なオープンソース Web アプリケーションファイアウォールです。完全なプライベートデプロイに対応し、データは暗号化してローカルに保存されます。起動も簡単で、Linux、Windows 64 ビット、ARM64 をサポートし、Docker イメージも提供されています。デフォルトでは外部依存ゼロの組み込み暗号化 SQLite データベースを使用し、必要に応じて MySQL / PostgreSQL へ切り替えることも可能です。
...
- デフォルトは SQLite（暗号化）、オプションで MySQL / PostgreSQL に対応。SQLite→MySQL / SQLite→PostgreSQL / MySQL→PostgreSQL 移行ツールを内蔵
...
| `migratedb` | SQLite → MySQL / SQLite → PostgreSQL / MySQL → PostgreSQL のオフラインデータベース移行（`--dry-run` で見積もりのみ、`--force` で上書き） |
