...
- SQLite (مشفَّرة) افتراضيًا مع خيار MySQL / PostgreSQL، وأداة مدمجة للترحيل: SQLite→MySQL / SQLite→PostgreSQL / MySQL→PostgreSQL
...
| `migratedb` | ترحيل قاعدة البيانات دون اتصال: SQLite → MySQL / SQLite → PostgreSQL / MySQL → PostgreSQL (`--dry-run` للتقدير فقط، و`--force` للاستبدال) |
