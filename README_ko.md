...
# 소프트웨어 소개
SamWaf는 소규모 회사, 스튜디오, 개인 웹사이트를 위한 경량 오픈소스 웹 애플리케이션 방화벽입니다. 완전한 프라이빗 배포를 지원하고, 데이터를 암호화하여 로컬에 저장하며, 손쉽게 시작할 수 있고, Linux, Windows 64비트 및 ARM64를 지원하며 Docker 이미지도 제공됩니다. 기본적으로 외부 의존성이 전혀 없는 내장 암호화 SQLite 데이터베이스를 사용하며, 필요에 따라 MySQL로 전환할 수 있습니다.
...
- 기본 SQLite (암호화), 선택적 MySQL, 내장 SQLite→MySQL 마이그레이션 도구 제공
...
| `migratedb` | SQLite → MySQL 오프라인 데이터베이스 마이그레이션 (`--dry-run`은 예상치만 산출, `--force`는 덮어쓰기) |
